package com.districthut.islam.prayer.scheduler;
...
public class SalaatAlarmReceiver extends WakefulBroadcastReceiver implements Constants {
  ...
  public void setAlarm(Context context) {
    ...
    SharedPreferences pref = context.getSharedPreferences("Salat", 0);
    SharedPreferences.Editor editor = pref.edit();
    editor.putString(EXTRA_PRAYER_NAME,nameOfPrayerFound);
    editor.putLong(EXTRA_PRAYER_TIME,then.getTimeInMillis());
    editor.apply();
...
//      removePassiveLocationUpdates(context, locationListenerPassivePendingIntent);
  ...
  public void showNotification(Context context,String prayerName) {
    Uri soundUri = Uri.parse("android.resource://" + context.getPackageName() + "/" + R.raw.adhan_trimmed);
    //Uri soundUri = Uri. parse (ContentResolver. SCHEME_ANDROID_RESOURCE + "://" + context.getPackageName() + "/raw/sound.mp3" );
    //  Uri soundUri = Uri. parse (ContentResolver. SCHEME_ANDROID_RESOURCE + "://" + context.getPackageName() + "/raw/adhan_trimmed.mp3" );
    ...
    NotificationCompat.Builder builder = new NotificationCompat.Builder(context, "MyNotifications")
            ...
            .setContentTitle("It's " + prayerName + " time.")
            //.setContentText("Much longer")
            // .setSound(soundUri)
            .setSound(soundUri, AudioManager.STREAM_NOTIFICATION)
            .setStyle(new NotificationCompat.BigTextStyle()
                    .bigText("And seek help in patience and prayer (Al-Baqara:45)"))
